...
def get_clinical_recommendation_stream(
    ...
    try:
        ...
        # Bước 4 & 5: Tạo prompt và gọi Agent Engine
        logger.info("📍 [Bước 4-5] Gọi Agent Engine để sinh tư vấn...")
        yield {"type": "status", "message": "Agent engine đang tạo tư vấn lâm sàng"}
...
{alternative_drugs_text}"""
        ...
        # Tạo query cho Agent Engine
        agent_query = f"""{fda_summary}
**YÊU CẦU:**
    ...
    """
        ...
        # Agent hiện trả về text đầy đủ, streamlit sẽ stream theo chunk để UI hiển thị dần.
        agent_text = run_clinical_agent(agent_query)
        if isinstance(agent_text, str) and agent_text.startswith("Lỗi khi chạy agent:"):
            result["error_message"] = agent_text
            yield {"type": "error", "message": agent_text, "result": result}
            return

        streamed_text = str(agent_text or "")
        max_stream_chars = 1500
        streamed_text = streamed_text[:max_stream_chars]

        chunk_size = 120
        for i in range(0, len(streamed_text), chunk_size):
            yield {"type": "recommendation_chunk", "chunk": streamed_text[i:i + chunk_size]}
