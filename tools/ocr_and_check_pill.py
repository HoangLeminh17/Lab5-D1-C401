...
# AGENT ENTRY POINT
def run_pipeline(image_path: str, inventory_path: str = str(INVENTORY_FILE)) -> dict:
    """
    OCR an invoice image and check drugs against inventory.
    Args:
        image_path:      Path to the drug invoice image.
        inventory_path:  Path to inventory.csv.
    Returns:
        {"drugs": list[dict], "results": list[dict], "error": str | None}
    """
